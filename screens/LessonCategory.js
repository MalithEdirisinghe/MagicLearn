...
const LessonCategory = ({ navigation, route }) => {
    ...
    const initializeLessons = () => {
        const initialLessons = lessonDataMap[category] || lessonDataMap['daily-routines'];
        return initialLessons.map((lesson) => ({ ...lesson, completed: false })); // Reset all to pending
    };

    useEffect(() => {
        setLessonData(initializeLessons());
    }, [category]);

    // Refresh handler to reset lessons to the initial state
    const onRefresh = () => {
        setRefreshing(true);
        setLessonData(initializeLessons()); // Reset to initial lesson data with all pending badges
        setRefreshing(false);
    };
